...
\item the set of complex numbers of the form $a + ai$, $a\in\R$ (under
  addition)
  \begin{proof}
    Call the set $H$. $H$ is obviously nonempty. For any $a + ai$ and
    $b + bi$ in $H$, we have
    \begin{equation*}
      (a + ai) - (b + bi) = (a - b) + (a - b)i\in H,
    \end{equation*}
    so by Proposition~1, $H\leq\C$.
  \end{proof}
\item the set of complex numbers of absolute value $1$, i.e., the unit
  circle in the complex plane (under multiplication)
  \begin{proof}
    Let $H$ denote the complex numbers of absolute value $1$, and let
    $\overline{z}$ denote the conjugate of $z$. Then $H$ is nonempty
    and for any $z,w\in H$, we have
    \begin{equation*}
      \abs{zw^{-1}} = \abs{z}\abs{w^{-1}}
      = \abs{z}\frac{\abs{\overline{w}}}{\abs{w}^2} = 1,
    \end{equation*}
    so $zw^{-1}\in H$. Therefore $H\leq\C^\times$.
  \end{proof}
\item for fixed $n\in\Z^+$ the set of rational numbers whose
  denominators divide $n$ (under addition)
  \begin{proof}
    Let $H$ denote the subset in question. $H$ is clearly not
    empty. Let $a/b\in H$ and $c/d\in H$ be in lowest terms, where
    $n = bx = dy$ for some $x,y\in\Z^+$. Then
    \begin{equation*}
      \frac{a}b - \frac{c}d
      = \frac{ax}n - \frac{cy}n
      = \frac{ax - cy}n.
    \end{equation*}
    After writing this fraction in lowest terms, its denominator will
    be some factor of $n$, so $(a/b - c/d)\in H$ as
    required. Therefore $H\leq\Q$.
  \end{proof}
\item for fixed $n\in\Z^+$ the set of rational numbers whose
  denominators are relatively prime to $n$ (under addition)
  \begin{proof}
    Again, let $H$ denote the subset, which is clearly nonempty. Take
    ...
    and $a/b - c/d \in H$. So $H\leq\Q$.
  \end{proof}
\item the set of nonzero real numbers whose square is a rational
  number (under multiplication)
  \begin{proof}
    Let $H$ be the set in question, which is clearly nonempty. If
    $a,b\in H$, then $a^2,b^2\in\Q$. Then
    \begin{equation*}
      \left(\frac{a}b\right)^2 = \frac{a^2}{b^2} \in \Q,
    \end{equation*}
    so $a/b\in H$. Hence $H\leq\R^\times$.
  \end{proof}
\end{enumerate}
...
\Exercise9
\label{exercise:SLn-is-subgroup}
...
\Exercise{10}
\label{exercise-intersection-of-subgroups-is-subgroup}
\begin{enumerate}
...
\Exercise{15} Let $H_1\leq H_2\leq\cdots$ be an ascending chain of
...
\Exercise{16} Let $n\in\Z^+$ and let $F$ be a field. Prove that the
set
\begin{equation*}
...
\Exercise{17} Let $n\in\Z^+$ and let $F$ be a field. Prove that the
set
\begin{equation*}
  \{(a_{ij})\in GL_n(F) \mid \text{$a_{ij} = 0$ for all $i > j$,

...
\Exercise2
\label{exercise:intro-group:perm2}
Let $\sigma$ be the permutation
\begin{align*}
...
\Exercise{16} Show that if $n\geq m$ then the number of $m$-cycles in
$S_n$ is given by
...
\Exercise{17} Show that if $n\geq4$ then the number of permutations in
...
\Exercise{18} Find all numbers $n$ such that $S_5$ contains an element
of order $n$.
\begin{solution}
...
\Exercise{19} Find all numbers $n$ such that $S_7$ contains an element
of order $n$.
\begin{solution}
...
\Exercise{20} Find a set of generators and relations for $S_3$.
\begin{solution}

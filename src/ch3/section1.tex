...
\Exercise{26}
\label{exercise:quotient-group:conjugate-properties}
Let $a,b\in G$.
\begin{enumerate}
...
\Exercise{32} Prove that every subgroup of $Q_8$ is normal. For each
...
\Exercise{33} Find all normal subgroups of $D_8$ and for each of these
...
\Exercise{34} Let $D_{2n} = \gen{r,s\mid r^n = s^2 = 1, rs = sr^{-1}}$
...
\Exercise{35} Prove that $SL_n(F)\trianglelefteq GL_n(F)$ and describe
...
\Exercise{37} Let $A$ and $B$ be groups. Show that
...
\Exercise{38} Let $A$ be an abelian group and let $D$ be the
...
\Exercise{39} Suppose $A$ is the non-abelian group $S_3$ and $D$ is
...
\Exercise{40} Let $G$ be a group, let $N$ be a normal subgroup of $G$
and let $\overline{G} = G/N$. Prove that $\bar{x}$ and $\bar{y}$
commute in $\overline{G}$ if and only if $x^{-1}y^{-1}xy\in N$. (The
element $x^{-1}y^{-1}xy$ is called the {\em commutator} of $x$ and $y$
and is denoted $[x,y]$.)
\begin{proof}
  ...
  and only if $a = yxzz_0$, if and only if $a\in yxN$. By symmetry,
  this shows that $\overline{xy} = \overline{yx}$.
\end{proof}
